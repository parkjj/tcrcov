...
immdata <- repLoad("./adaptiveHD-filtered")
meta <- fread("./adaptiveHD-filtered/metadata.txt")

immdata_cod <- coding(immdata$data)
immdata$data <- immdata_cod

saveRDS(immdata, "adaptiveHD_immdata.rds")
#immdata <- readRDS("adaptiveHD_immdata.rds")
...
write.csv(exp_clones,"./immunarch_outputs/immunarch_stats/clonotype_stats_clones_adaptiveHD.csv" ,quote = F,  row.names = F)

exp_len <- repExplore(immdata$data, .method = "len")
exp_len <-  cbind(exp_len, meta[match(exp_len$Sample, meta$Sample),])
write.csv(exp_len,"./immunarch_outputs/immunarch_stats/clonotype_stats_len_adaptiveHD.csv" ,quote = F,  row.names = F)


exp_vol <- repExplore(immdata$data, .method = "volume", .col = "aa")
exp_vol <-  cbind(exp_vol, meta[match(exp_vol$Sample, meta$Sample),])
write.csv(exp_vol,"./immunarch_outputs/immunarch_stats_aa/clonotype_stats_total_aa_adaptiveHD.csv" ,quote = F,  row.names = F)

exp_clones <- repExplore(immdata$data, .method = "clones", .col = "aa")
exp_clones <-  cbind(exp_clones, meta[match(exp_clones$Sample, meta$Sample),])
write.csv(exp_clones,"./immunarch_outputs/immunarch_stats_aa/clonotype_stats_clones_aa_adaptiveHD.csv" ,quote = F,  row.names = F)

exp_len <- repExplore(immdata$data, .method = "len", .col = "aa")
exp_len <-  cbind(exp_len, meta[match(exp_len$Sample, meta$Sample),])
write.csv(exp_len,"./immunarch_outputs/immunarch_stats_aa/clonotype_stats_len_aa_adaptiveHD.csv" ,quote = F,  row.names = F)
...
write.csv(div_gini,"./immunarch_outputs/immunarch_stats/diversity_stats_gini_adaptiveHD.csv" ,quote = F,  row.names = F)

div_invsimp <- repDiversity(immdata$data, "inv.simp")
div_invsimp <-  cbind(div_invsimp, meta[match(div_invsimp$Sample, meta$Sample),])
write.csv(div_invsimp,"./immunarch_outputs/immunarch_stats/diversity_stats_invsimp_adaptiveHD.csv" ,quote = F,  row.names = F)