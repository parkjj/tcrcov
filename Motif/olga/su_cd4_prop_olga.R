...
for(i in c("all", "hd", "mild", "moderate", "severe")){
  input <- fread(paste0("su_cd4_gliph2_clones_",i,"_cluster.csv"), fill = T)
  beta <- input$TcRb
  v <- input$V
  j <- input$J
  df <- data.frame(beta, v, j)
  df <- df[-which(df$beta == ""), ]
  write.table(df, paste0("su_cd4_clones_",i,"_cluster_olgainput.tsv"), quote = F, col.names = F, row.names = F, sep = "\t")
...
input <- fread(paste0("su_cd4_gliph2_clones_",i,"_cluster.csv"), fill = T)
input <- input[-which(input$TcRb == ""), ]
pgenscores <- fread(paste0("su_cd4_clones_",i,"_cluster_pgens.tsv"))
...
hits[which(dfplt$freq > 0.013)] <- "hit"
dfplt$hits <- hits
...
pdf(paste0("clusterplots/su_cd4_clones_",i,"_clusterplot.pdf"), height = 8, width = 10)
plt
dev.off()
...
fwrite(pooled_master, "clusterplots/su_cd4_clones_clonotypes_all.csv")
...
pooled_master <- pooled_master[which(pooled_master$pattern %in% unique(pooled_master$pattern)[1:40]),]
...
plt <- plt + facet_wrap(~ pattern, ncol=8)
plt <- plt  + labs(x = "Clonotype frequency", y = "-log2( Clonotype pGen score )")
plt <- plt + scale_size(breaks = 1:5, range = c(3,15))
#plt <- plt + scale_x_continuous(breaks = pretty_breaks(n=7), limits = c(0,0.2))
pdf(paste0("clusterplots/su_cd4_clones_clonotypes_facet.pdf"), height = 8, width = 14)
plt
dev.off()

fwrite(pooled_master, "clusterplots/su_cd4_clones_clonotypes_dzspecific.csv")
...
pdf("clusterplots/su_cd4_clones_clonotypes_dz_venn.pdf", height = 6, width = 6)
...
pdf("clusterplots/su_cd4_clones_clonotypes_dz_specific_venn.pdf", height = 6, width = 6)
grid.newpage()
venn.plot <- draw.pairwise.venn(area1      = length(pattern_hd),
                                ...
                                scaled     = FALSE)
dev.off()
write.table(pattern_dz_specific, "clusterplots/su_cd4_clones_clonotypes_dz_specific_venn_setdiff.csv", quote = F, row.names = F, col.names = F)
