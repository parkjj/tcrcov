...
metadata <- fread("../vdj_datasets/su_cd8_reform/metadata.txt")

file.names <- dir("../vdj_datasets/su_cd8_reform", pattern =".txt")
...
for (i in 1:length(file.names)){
  print(i)
  file <- fread(paste0("../vdj_datasets/su_cd8_reform/", file.names[i]))
...
fwrite(kmermaster,"su_cd8_7mers.csv", quote = F)
...
write.table(pc, paste0("su_cd8_pca_",kmersize,"mer.csv"), quote = F, sep = ",", row.names = F, col.names = T)
...
qcpc <- qcpc + scale_shape_manual(values=c(21, 2, 5)) + ggtitle(paste0("PCA ISB-S CD8 dataset, ",kmersize,"-mers"))
pdf(paste0("su_cd8_pca_",kmersize,"mer.pdf"), height = 5, width = 8)
qcpc
dev.off()
pdf(paste0("su_cd8_pca_",kmersize,"mer_scree.pdf"), height = 5, width = 7)
fviz_eig(pcoutput, addlabels = T, barfill = "#6E85B2",barcolor = "#261C2C") + theme(text = element_text(size = 16), axis.title = element_text(size = 16), axis.text = element_text(size = 16))+ ggtitle(paste0("Scree plot, ISB-S CD8 dataset, ",kmersize,"-mers"))
dev.off()
...
pdf(paste0("dex/hm_ph_su_cd8_",kmersize,"mer.pdf"), width = 16, height = 11.5)
...
pdf(paste0("dex/hm_su_cd8_",kmersize,"mer.pdf"), width = 16, height = 11.5)
plt
dev.off()